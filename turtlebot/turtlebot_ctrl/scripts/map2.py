...
boundary = [(8,8), (8,-7) ,(-7,-7), (-7,8)]
obstacle1 = [(-1,-1), (-6,-2), (-5,2), (-3,2) ,(-4,0)]
obstacle2 = [(6,5), (4,1), (5,-2), (2,-4), (1,2) ]
obstacle3 = [(0,-3),(0,-4), (1,-5), (-5,-5), (-5,-4) ]
obstacle4 = [(7,6), (0,4), (-5,6), (0,6), (4,7)]
...
def generateParticles(self,n):
    ...
    #have to set it back to curryaw
    for i in range(n):
        x = random.uniform(-7, 8)
        y = random.uniform(-7,8)
        if inside_polygon(x,y, obstacle1) or inside_polygon(x,y, obstacle2) or inside_polygon(x,y, obstacle3) or inside_polygon(x,y,obstacle4):
            continue
...
def updateparticlesXY(self, distance):
	...
	newy=0
    for key in particles.keys():
        if inside_polygon(key[0], key[1], obstacle1) or inside_polygon(key[0], key[1], obstacle2) or inside_polygon(key[0], key[1], obstacle3) or inside_polygon(key[0], key[1], obstacle4)or outside_polygon(key[0], key[1], boundary):
            del particles[key]
            continue
        x = key[0]
        y = key[1]
...
def iterateshow(self):
    ...
    plt.plot([8, 8, -7, -7, 8], [8, -7, -7, 8, 8], 'b-')

    # obstacle#1
    plt.plot([-1, -6, -5, -3, -4,-1], [-1, -2, 2, 2, 0,-1], 'c-')
    # obstacle#2
    plt.plot([6, 4, 5, 2,1,6], [5, 1, -2, -4,2,5], 'c-')
    # obstacle#3
    plt.plot([0, 0, 1, -5, -5, 0], [-3, -4, -5, -5, -4, -3], 'c-')
    #obstacle#4
    plt.plot([7,0,-5,0,4,7], [6,4,6,6,7,6], 'c-')
    listofweight = []
    for key in particles.keys():
