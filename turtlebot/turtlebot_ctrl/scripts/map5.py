...
boundary = [(10,10), (10,-9), (-9,-9), (-9,10)]
obstacle1 = [(7,8), (0,1), (1,3), (6,8)]
obstacle2 = [(10,5), (8,3), (6,5), (7,6)]
obstacle3 = [(4,4), (8,1), (8,0), (4,3)]
obstacle4 = [(9,2), (10,5), (10,-1)]
obstacle5 = [(2,2), (-2,-9), (-3,-8), (1,2)]
obstacle6 = [(-3,-9), (-6,-8), (-7,-3)]
obstacle7 = [(-7,-3), (-9,0), (-8,1)]
obstacle8 = [(-6,5), (-1,2), (-2,-4), (-5,-2), (-4,1)]
obstacle9 = [(10,-3), (7,-7), (6,-6), (6,0)]
obstacle10 = [(-7,10), (-6,10), (-6,7), (-7,6)]
obstacle11 = [(0,7), (5,10), (5,9), (0,6)]
obstacle12 = [(4,-3), (4,-6), (2,-6), (2,-3)]
obstacle13 = [(-4,8), (0,5), (0,4), (-5,7)]
...
def generateParticles(self,n):
    ...
    for i in range(n):
        ...
        y = random.uniform(-9,10)
        if inside_polygon(x,y, obstacle1) or inside_polygon(x,y, obstacle2) or inside_polygon(x,y, obstacle3) or inside_polygon(x,y,obstacle4) or inside_polygon(x,y,obstacle5)or inside_polygon(x,y, obstacle6)or inside_polygon(x,y, obstacle7)or inside_polygon(x,y, obstacle8)or inside_polygon(x,y, obstacle9)or inside_polygon(x,y, obstacle10)or inside_polygon(x,y, obstacle11)or inside_polygon(x,y, obstacle12)or inside_polygon(x,y, obstacle13):
            continue
...
def updateparticlesXY(self, distance):
	...
	newy=0
    for key in particles.keys():
        if inside_polygon(key[0], key[1], obstacle1) or inside_polygon(key[0], key[1], obstacle2) or inside_polygon(key[0], key[1], obstacle3) or outside_polygon(key[0], key[1], boundary)  or inside_polygon(key[0], key[1], obstacle4) or inside_polygon(key[0], key[1], obstacle5) or inside_polygon(key[0], key[1], obstacle6) or inside_polygon(key[0], key[1], obstacle7) or inside_polygon(key[0], key[1], obstacle8) or inside_polygon(key[0], key[1], obstacle9) or inside_polygon(key[0], key[1], obstacle10) or inside_polygon(key[0], key[1], obstacle11)or inside_polygon(key[0], key[1], obstacle12)or inside_polygon(key[0], key[1], obstacle13):
            del particles[key]
            continue
        x = key[0]
        y = key[1]
...
def iterateshow(self):
    ...
    plt.plot([10, 10, -9, -9, 10], [10, -9, -9, 10, 10], 'b-')

    # obstacle#1
    plt.plot([7, 0, 1, 6, 7], [8, 1, 3, 8, 8], 'c-')
    # obstacle#2
    plt.plot([10, 8, 6, 7, 10], [5, 3, 5, 6, 5], 'c-')
    # obstacle#3
    plt.plot([4, 8, 8, 4, 4], [4, 1, 0, 3, 4], 'c-')
    #obstacle#4
    plt.plot([9,10,10,9], [2,5,-1,2], 'c-')
    # obstacle#5
    plt.plot([2,-2,-3,1,2], [2,-9,-8,2,2], 'c-')
    # obstacle#6
    plt.plot([-3,-6,-7,-3], [-9,-8,-3,-9], 'c-')
    # obstacle#7
    plt.plot([-7,-9,-8,-7], [-3,0,1,-3], 'c-')
    #obstacle#8
    plt.plot([-6,-1,-2,-5,-4,-6], [5,2,-4,-2,1,5], 'c-')
    # obstacle#9
    plt.plot([10,7,6,6,10], [-3,-7,-6,0,-3], 'c-')
    # obstacle#10
    plt.plot([-7,-6,-6,-7,-7], [10,10,7,6,10], 'c-')
    #obstacle#11
    plt.plot([-4,0,0,-5,-4], [8,5,4,7,8], 'c-')
    #obstacle#12
    plt.plot([0,5,5,0,0], [7,10,9,6,7], 'c-')
    #obstacle#13
    plt.plot([4,4,2,2,4], [-3,-6,-6,-3,-3], 'c-')
    listofweight = []
    for key in particles.keys():
