...
obstacle1 = [(-9,6),(-3,6), (-2,5), (-9,5)]
obstacle2 = [(-3,6),(-3,8), (-2,8), (-2,5) ]
obstacle3 = [(-3,5),(-3,1), (-4,1), (-4,5) ]
obstacle4 = [(10,9), (10,8), (3,7), (2,8)]
obstacle5 = [(7,6), (8,5), (1,4), (0,5)]
obstacle6 = [(10,3), (10,2), (3,1), (2,2)]
obstacle7 = [(9,-2), (9,-8), (8,-8), (8,-2)]
obstacle8 = [(8,-2), (8,-3), (-1,-5), (-1,-4)]
obstacle9 = [(-1,-4), (-2,-5), (-7,1), (-7,2)]
obstacle10 = [(-3,-4), (-3,-5), (-7,-7), (-7,-6)]
obstacle11 = [(0,3),(3,-2), (-2,0)]
...
def generateParticles(self,n):
    ...
    for i in range(n):
        ...
        if inside_polygon(x,y, obstacle1) or inside_polygon(x,y, obstacle2) or inside_polygon(x,y, obstacle3) or inside_polygon(x,y,obstacle4) or inside_polygon(x,y,obstacle5)or inside_polygon(x,y, obstacle6)or inside_polygon(x,y, obstacle7)or inside_polygon(x,y, obstacle8)or inside_polygon(x,y, obstacle9)or inside_polygon(x,y, obstacle10)or inside_polygon(x,y, obstacle11):
            continue
...
def updateparticlesXY(self, distance):
	...
	newy=0
    for key in particles.keys():
        if inside_polygon(key[0], key[1], obstacle1) or inside_polygon(key[0], key[1], obstacle2) or inside_polygon(key[0], key[1], obstacle3) or outside_polygon(key[0], key[1], boundary)  or inside_polygon(key[0], key[1], obstacle4) or inside_polygon(key[0], key[1], obstacle5) or inside_polygon(key[0], key[1], obstacle6) or inside_polygon(key[0], key[1], obstacle7) or inside_polygon(key[0], key[1], obstacle8) or inside_polygon(key[0], key[1], obstacle9) or inside_polygon(key[0], key[1], obstacle10) or inside_polygon(key[0], key[1], obstacle11):
            del particles[key]
            continue
        x = key[0]
        y = key[1]
...
def iterateshow(self):
    ...
    plt.axis([-19, 19, -19, 19])

    # this is ploting the boundary of the map
    plt.plot([10, 10, -9, -9, 10], [10, -9, -9, 10, 10], 'b-')

    # obstacle#1
    plt.plot([-9, -3, -2, -9, -9], [6, 6, 5, 5, 6], 'c-')
    # obstacle#2
    plt.plot([-3, -3, -2, -2,-3], [6, 8, 8, 5,6], 'c-')
    # obstacle#3
    plt.plot([-3, -3, -4, -4, -3], [5, 1, 1, 5, 5], 'c-')
    #obstacle#4
    plt.plot([10,10,3,2,10], [9,8,7,8,9], 'c-')
    # obstacle#5
    plt.plot([7,8,1,0,7], [6,5,4,5,6], 'c-')
    # obstacle#6
    plt.plot([10,10,3,2,10], [3,2,1,2,3], 'c-')
    # obstacle#7
    plt.plot([9,9,8,8,9], [-2,-8,-8,-2,-2], 'c-')
    #obstacle#8
    plt.plot([8,8,-1,-1,8], [-2,-3,-5,-4,-2], 'c-')
    # obstacle#9
    plt.plot([-1,-2,-7,-7,-1], [-4,-5,1,2,-4], 'c-')
    # obstacle#10
    plt.plot([-3,-3,-7,-7,-3], [-4,-5,-7,-7,-4], 'c-')
    #obstacle#11
    plt.plot([0,3,-2,0], [3,-2,0,3], 'c-')
    listofweight = []
    for key in particles.keys():
