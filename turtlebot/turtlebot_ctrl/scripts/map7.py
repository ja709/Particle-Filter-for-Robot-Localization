...
obstacle1 = [(-3,0), (-4,6), (-7,8), (0,6), (3,6), (1,4), (-2,5)]
obstacle2 = [(0,2), (3,4), (4,-2), (2,0), (0,-2), (-1,0)]
obstacle3 = [(-7,-4), (-4,-2), (0,-5), (5,-3), (4,-6), (0,-7), (-4,-4)]
obstacle4 = [(-8,-8), (-8,-5), (-2,-8)]
obstacle5 = [(5,1), (6,7), (7,8), (8,6), (7,5), (8,3), (6,4), (6,1)]
obstacle6 = [(7,-4), (5,-1), (8,-3), (8,-5), (7,-5), (7,-9), (5,-7)]
...
def generateParticles(self,n):
    ...
    for i in range(n):
        ...
        if inside_polygon(x,y, obstacle1) or inside_polygon(x,y, obstacle2) or inside_polygon(x,y, obstacle3) or inside_polygon(x,y,obstacle4) or inside_polygon(x,y,obstacle5)or inside_polygon(x,y, obstacle6):
            continue
...
def updateparticlesXY(self, distance):
	...
	newy=0
    for key in particles.keys():
        if inside_polygon(key[0], key[1], obstacle1) or inside_polygon(key[0], key[1], obstacle2) or inside_polygon(key[0], key[1], obstacle3) or outside_polygon(key[0], key[1], boundary)  or inside_polygon(key[0], key[1], obstacle4) or inside_polygon(key[0], key[1], obstacle5) or inside_polygon(key[0], key[1], obstacle6):
            del particles[key]
            continue
        x = key[0]
        y = key[1]
...
def iterateshow(self):
    ...
    # obstacle#1
    plt.plot([-3, -4, -7, 0, 3,1,-2,-3], [0, 6, 8, 6, 6,4,5,0], 'c-')
    # obstacle#2
    plt.plot([0, 3, 4, 2,0,-1,0], [2, 4, -2, 0,-2,0,2], 'c-')
    # obstacle#3
    plt.plot([-7, -4, 0, 5, 4,0,-4,-7], [-4, -2, -5, -3, -6,-7,-4,-4], 'c-')
    #obstacle#4
    plt.plot([-8,-8,-2,-8], [-8,-5,-8,-8], 'c-')
    # obstacle#5
    plt.plot([5,6,7,8,7,8,6,6,5], [1,7,8,6,5,3,4,1,1], 'c-')
    # obstacle#6
    plt.plot([7,5,8,8,7,7,5,7], [-4,-1,-3,-5,-5,-9,-7,-4], 'c-')
    listofweight = []
    for key in particles.keys():
